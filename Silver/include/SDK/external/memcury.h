...
DEFINE_LOG_CATEGORY(LogMemory, All);
...
namespace Memcury
{
    ...
    public:
        ...
        static auto FindPattern(const char* signature, bool bShouldWarn = false) -> Scanner
        {
            ...
            if (bShouldWarn)
            {
                std::string TempStr = std::string(signature);
                std::wstring TempWStr = std::wstring(TempStr.begin(), TempStr.end());
                UE_LOG(LogMemory, Warning, L"FindPattern failed for signature: %s", TempWStr.c_str());
        ...
        static auto FindStringRef(T string, bool bShouldWarn = false, bool find_first = false) -> Scanner
        {
			...
			if (bShouldWarn && bIsWide)
			{
                UE_LOG(LogMemory, Warning, L"Failed to find String: %s", string);
